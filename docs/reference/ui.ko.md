...
이 섹션에는 AI 준수 개요 카드도 함께 나타납니다. G7 결과 요약, 라이선스 표시 컴포넌트 수, 규제 프레임워크 커버리지(EU AI Act, 한국 AI 기본법)를 한눈에 보여줍니다. 인증이 아니라 참고용이며, 같은 데이터가 `_ai-profile` 파일에도 기록됩니다([산출물](artifacts.ko.md) 참고).

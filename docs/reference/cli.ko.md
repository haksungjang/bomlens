...
같은 `--project`/`--version`으로 다시 스캔하면 같은 출력 폴더를 재사용합니다. 그 폴더의 이전 스캔이 끝나지 못하고(강제 종료, 메모리 부족, 호스트 크래시) 오늘 실행의 모드나 옵션으로는 만들어지지 않을 파일을 남겼다면, 다음 스캔이 자신의 산출물을 쓰기 전에 그 파일부터 지웁니다. 그래서 이번 실행의 결과로 착각할 만한 것이 남지 않습니다.

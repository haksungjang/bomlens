...
| 5 | `--fail-on`: 스캔은 성공했지만 조건을 판정할 수 없음(예: 취약점 데이터베이스를 내려받지 못함, `--license` 없이 `license-conflict`), 또는 판정할 결과가 안 만들어짐 |

...
한 번 준비해 두면, 락 파일이 이미 커밋된 프로젝트는 앱을 열고 스캔하는 데 1~2분이면 됩니다. 락 파일이 없으면 cdxgen이 스캔 전에 의존성을 직접 해석하는데([웹 UI 참조](../reference/ui.ko.md#new-scan) 참고), `package-lock.json`이 없는 Node.js 프로젝트로 반복 실측한 결과 6~7분이 걸렸습니다.

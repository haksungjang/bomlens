...
구조상 이 경로가 할 수 없는 일이 둘 있습니다. 모델 카드가 없으므로 학습 데이터와 사용 목적, 평가를 묻는 G7 항목은 공백으로 보고됩니다. 파일이 그에 관해 아무 말도 하지 않으니 정확한 보고입니다. 그리고 파일 하나를 기술하므로, 샤드 여러 개로 나뉜 모델은 지금은 샤드마다 따로 스캔합니다.

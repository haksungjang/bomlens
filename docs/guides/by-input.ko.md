...
- **적합성 보고서**: SBOM이 제출 품질 기준(필수 항목, PURL 보유율, 전이 의존성 등)을 충족하는지 항목별로 점검한 결과입니다. 다른 세 산출물과 함께 기본 생성됩니다. 자세한 읽는 법은 공급사 SBOM 검증 가이드의 [적합성 보고서 읽기](supplier-sbom.ko.md#적합성-보고서-읽기)를 참고하세요.
...
실행하면 진행 로그가 실시간으로 표시되고, 완료 후에는 고지문과 SBOM, 위험분석보고서, 적합성 보고서를 화면에서 보거나 내려받을 수 있습니다. 적합성 결과(적합/부적합)는 상단 카드로 표시됩니다.

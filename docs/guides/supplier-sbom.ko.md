...
SPDX 2.2도 읽지만 담기는 정보가 적습니다. Yocto 4.0 Kirkstone과 5.0 Scarthgap은 기본값이 SPDX 2.2인데, 이 형식은 파일 하나가 아닙니다. 최상위 `.spdx.json`은 색인이고 실제 패키지는 옆에 있는 `.spdx.tar.zst` 안의 문서들에 들어 있습니다. 이 둘을 함께 읽으면 설치 패키지와 라이선스, CPE를 얻습니다.

없는 것은 빌드의 CVE 판정입니다. 레시피가 어떤 CVE를 패치했는지는 SPDX 3.0에만 기록되므로, 취약점은 다른 SBOM과 마찬가지로 CPE로 매칭합니다. 그래서 빌드가 이미 패치한 CVE가 열린 것으로 보일 수 있습니다. 색인 파일만 따로 올리면 결과가 거의 비는데, 이때는 깨끗한 스캔처럼 보이게 두지 않고 그 사실을 알려 줍니다. 빌드 디렉터리에 두 형식이 함께 있으면 2.2 쪽이 더 최근이더라도 SPDX 3.0 문서를 분석합니다.

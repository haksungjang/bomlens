...
빌드 디렉터리로 인식하고, 빌드가 만든 이미지 SBOM인 `tmp/deploy/images/<machine>/<image>.rootfs.spdx.json`(OpenEmbedded 빌드는 `tmp-glibc/…`)을 분석합니다. 이미지를 완전히 다른 위치에 쓰도록 `DEPLOY_DIR`를 옮긴 빌드는 이 방법으로 찾지 못하므로, 그 문서를 `--analyze <파일>`로 직접 지정합니다. 빌드 트리 자체는 훑지 않습니다. 디렉터리로 스캔하면 이미지에 들어가지 않는 sysroot와 빌드용 도구까지 결과에 섞이기 때문입니다. 머신이나 이미지가 여러 개라 SBOM이 여럿이면 가장 최근에 쓰인 것을 분석하고 후보 전체를 로그에 남깁니다. 다른 것을 고르려면 `--analyze <파일>`로 직접 지정합니다. 이 파일 하나만 따로 올려도 됩니다. 웹 UI 업로드나 `--analyze`가 그 경로이고, SBOM만 받은 경우에 씁니다.

Yocto 빌드 디렉터리인데 SPDX 문서가 없으면 디렉터리 스캔으로 넘어가지 않고 스캔을 멈춘 뒤 위 설정 두 줄을 안내합니다. 그대로 스캔하면 이미지 내용과 다른 결과가 나오기 때문입니다.
...
SPDX 3.0만 지원합니다. Yocto 4.0 Kirkstone과 5.0 Scarthgap은 기본값이 SPDX 2.2인데, 이 형식은 파일 하나가 아닙니다. 최상위 `.spdx.json`은 색인이고 실제 패키지는 옆에 있는 `.spdx.tar.zst` 안의 레시피별 문서에 들어 있습니다. 이 경우 빈 결과를 조용히 내놓지 않고 그 사실을 알려 줍니다. 빌드 디렉터리를 지정했다면 스캔 전에, 색인 파일만 올렸다면 스캔 중에 알립니다. 빌드 디렉터리에 두 형식이 함께 있으면 2.2 쪽이 더 최근이더라도 SPDX 3.0 문서를 분석합니다. 두 릴리스 모두 위 설정으로 SPDX 3.0을 만들 수 있습니다.

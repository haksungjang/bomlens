...
- The build must have been configured to emit an SBOM. Add `INHERIT += "create-spdx-3.0"` and `INHERIT += "vex"` to `conf/local.conf` and build as usual; if the directory holds no SPDX document the scan stops and says so.

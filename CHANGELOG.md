...
- The web UI and the desktop app can now hand a problem over: the result screen of a finished scan, succeeded or failed, has a **Report a problem** panel that shows a diagnostics summary (versions, container engine, options, per-stage results, warnings; no source, paths of yours, or credentials) and copies it in one press, the run log has a **Copy log** button, and the Help menu links to a new issue form that asks for the command, scan mode, container engine and image tag. Nothing is sent anywhere.

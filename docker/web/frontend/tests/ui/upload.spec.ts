...
// tests/test-web-ui.sh and the real path by tests/integration.
const UPLOAD_DONE = {
  ok: true,
  mode: "SOURCE",
...
test("ZIP upload flow uploads then renders the scan result", async ({ page }) => {
  await stubBackend(page, { caps: { scanoss: true }, upload: { token: "tok123", filename: "demo.zip" }, done: UPLOAD_DONE });
  let uploaded = false;
  page.on("request", (req) => {
...
test("a failed upload surfaces an error instead of running the scan", async ({ page }) => {
  await stubBackend(page, { caps: { scanoss: true }, upload: { status: 413, error: "file too large for zip" } });
  await page.goto("/");
  await selectZipAndAttach(page);
  await page.getByRole("button", { name: /Run scan/i }).click();
